...
class ReadinessTestCase(TestCase):
    ...
    def test_readiness_file_is_not_present_after_exiting_context(self):
        with Readiness(self.test_readiness_file_path):
            pass
        assert not os.path.isfile(self.test_readiness_file_path), 'Readiness file was still present after exiting ' \
                                                                  'context'
    ...
    def test_readiness_file_is_not_present_after_uncaught_exception_in_context(self):
        with self.assertRaises(Exception):
            with Readiness(self.test_readiness_file_path):
                raise Exception
        assert not os.path.isfile(self.test_readiness_file_path), 'Readiness file was still present after uncaught ' \
