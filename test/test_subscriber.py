...
def test_subscription_set_up():
    with patch('app.subscriber.client') as dummy_client:
        dummy_client.topic_path = create_when_then_return(RECEIPT_TOPIC_PROJECT_ID, RECEIPT_TOPIC_NAME,
                                                          return_value=TOPIC_PATH)
        dummy_client.subscription_path = create_when_then_return(SUBSCRIPTION_PROJECT_ID, SUBSCRIPTION_NAME,
                                                                 return_value=SUBSCRIPTION_PATH)
        dummy_client.create_subscription = create_when_then_return(SUBSCRIPTION_PATH, TOPIC_PATH, return_value=None)
        dummy_client.subscribe = create_when_then_return(SUBSCRIPTION_PATH, callback_func,
